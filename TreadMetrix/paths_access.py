...
from TreadMetrix.paths_back import get_local, MissingLoadbearingPathException
from TreadMetrix.paths_gui import missing_loadbearing_path
...
def get_raw_mot_path() -> str | list[str]:
    ...
    return content if content is not None else os.path.join(get_local("base_path"), 'raw')


def get_corrected_mot_path() -> str:
    content = get_local('corrected_mot')
    return content if content is not None else os.path.join(get_local("base_path"), 'corrected_mot')


def get_segmented_mot_path() -> str:
    content = get_local('segmented_mot')
    path = content if content is not None else os.path.join(get_local("base_path"), 'segmented')
...
def get_external_loads_path() -> str:
    ...
    path = content if content is not None else os.path.join(get_local("base_path"), 'external_loads')
    os.makedirs(path, exist_ok=True)
    return path
...
def get_ik_results_path() -> str:
    ...
    path = content if content is not None else os.path.join(get_local("base_path"), 'ik_results')
    os.makedirs(path, exist_ok=True)
    return path
...
def get_id_results_path() -> str:
    ...
    path = content if content is not None else os.path.join(get_local("base_path"), 'id_results')
    os.makedirs(path, exist_ok=True)
    return path
...
def get_power_filtered_path() -> str:
    ...
    path = content if content is not None else os.path.join(get_local("base_path"), 'power_filtered')
    os.makedirs(path, exist_ok=True)
    return path
