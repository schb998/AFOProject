    ...
    def load(cls, filepath, filename=None, header=True, delimiter="\t"):
        ...
        try:
            with open(filepath, 'r') as file:
                ...
                return cls(filename, meta_data, marker_set, sub_headers, marker_dictionary, data, first_frame, file_header if header else None)
        except OSError as e:
...
class Test:
    ...
    def _test_load_all():
        ...
        for f in trcs:
            assert f == TRC.load(os.path.join(path, f.filename)), "Mass loaded objects should match object loaded from the same file."
